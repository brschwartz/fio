...
bool calc_thread_status(struct jobs_eta *je, int force)
{
	...
	if (write_bw_log && rate_time > bw_avg_time && !in_ramp_time(td)) {
		...
		add_agg_sample(sample_val(je->rate[DDIR_READ]), DDIR_READ, 0, td->priorityBit);
		add_agg_sample(sample_val(je->rate[DDIR_WRITE]), DDIR_WRITE, 0, td->priorityBit);
		add_agg_sample(sample_val(je->rate[DDIR_TRIM]), DDIR_TRIM, 0, td->priorityBit);
	}
